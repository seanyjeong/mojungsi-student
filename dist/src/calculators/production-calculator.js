...
exports.calculateScoreWithConv = calculateScoreWithConv;
...
function isSubjectUsedInRules(name, rulesArr) {
    const rules = Array.isArray(rulesArr)
        ? rulesArr
        : rulesArr
            ? [rulesArr]
            : [];
...
function buildSpecialContext(F, S, highestMap) {
    ...
    if (F.english_scores && S.영어?.grade != null) {
        ...
        const vals = Object.values(F.english_scores)
            .map(Number)
            .filter((n) => !Number.isNaN(n));
        ...
        ctx.eng_pct_est =
            engMax > 0
                ? Math.min(100, Math.max(0, (ctx.eng_grade_score / engMax) * 100))
                : 0;
    }
    else {
        ctx.eng_pct_est = 0;
    ...
    ctx.inq_avg2_converted_std =
        ctx.inq_sum2_converted_std /
            (sortedConv.length >= 2 ? 2 : sortedConv.length || 1);
    ...
    ctx.top2_sum_scaled_kme =
        (items_scaled_kme[0] || 0) + (items_scaled_kme[1] || 0);
    ...
    const cfg2 = safeParse(F.score_config, {});
    if (cfg2?.math_bonus_cap_100 === true) {
    ...
    ctx.top2_sum_norm_pct_kmi2 =
        ((items_pct[0] || 0) + (items_pct[1] || 0)) / 100;
...
function calculateScore(formulaDataRaw, studentScores, highestMap) {
    ...
    if (F.history_scores && typeof F.history_scores === 'object') {
        const vals = Object.values(F.history_scores)
            .map(Number)
            .filter((n) => !Number.isNaN(n));
        if (vals.length)
        ...
        if (name === '탐구' && inqMethod === 'highest_of_year') {
            const allInquiryNormalized = (S.탐구 || [])
                ...
                if (inqType === '변환표준점수') {
                    ...
                    if (!convTableForGroup ||
                        Object.keys(convTableForGroup).length === 0)
                        return null;
                    const maxConvScore = Math.max(...Object.values(convTableForGroup)
                        .map(Number)
                        .filter((n) => !isNaN(n)));
            ...
            if (pickedNormalized.length) {
                const avg = pickedNormalized.reduce((s, r) => s + r.normalized, 0) /
                    pickedNormalized.length;
    ...
    const selectNRules = rules.filter((r) => r?.type === 'select_n' && Array.isArray(r.from) && r.count != null);
...
function calculateScoreWithConv(formulaDataRaw, studentScores, convMap, logHook, highestMap) {
    ...
    const inqType = cfg?.inquiry?.type || '백분위';
    if (inqType === '변환표준점수' &&
        Array.isArray(studentScores?.subjects)) {
...
//# sourceMappingURL=production-calculator.js.map